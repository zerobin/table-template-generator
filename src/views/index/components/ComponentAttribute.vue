<template>
  <div v-if="isShow">
    <el-form size="small" label-width="90px">
      ...
      <el-form-item v-if="activeData.__config__.tag === 'el-table'" label="标签宽度">
        <el-input v-model.number="activeData.__config__.labelWidth" type="number" placeholder="请输入标签宽度" />
      </el-form-item>
      <!-- 容器组件的form属性 -->
      <el-form-item v-if="activeData.__config__.layout==='rowFormItem'" label="容器类型">
        <el-select v-model="activeData.__config__.containerType" placeholder="请选择水平排列" :style="{width: '100%'}">
          <el-option
      ...
      <template v-if="activeData.__config__.layout==='rowFormItem' && activeData.__config__.containerType === 'form'">
        <el-form-item label="表单名">
          <el-input v-model="activeData.__config__.form.formRef" placeholder="请输入表单名（ref）" />
        </el-form-item>
        <el-form-item label="表单模型">
          <el-input v-model="activeData.__config__.form.formModel" placeholder="请输入数据模型" />
        </el-form-item>
        <el-form-item label="表单尺寸">
          <el-radio-group v-model="activeData.__config__.form.size">
        ...
        <el-form-item label="标签对齐">
          <el-radio-group v-model="activeData.__config__.form.labelPosition">
        ...
        <el-form-item label="标签宽度">
          <el-input v-model.number="activeData.__config__.form.labelWidth" type="number" placeholder="请输入标签宽度" />
        </el-form-item>
        <el-form-item label="栅格间隔">
          <el-input-number v-model="activeData.__config__.form.gutter" :min="0" placeholder="栅格间隔" />
        </el-form-item>
        <el-form-item label="禁用表单">
          <el-switch v-model="activeData.__config__.form.disabled" />
        </el-form-item>
        <el-form-item label="表单按钮">
          <el-switch v-model="activeData.__config__.form.formBtns" />
        </el-form-item>
        <el-form-item label="显示未选中组件边框">
          <el-switch v-model="activeData.__config__.form.unFocusedComponentBorder" />
        </el-form-item>
      </template>
      ...
      <el-form-item v-if="activeData.__config__.labelWidth !== undefined && activeData.__config__.tag !== 'el-table'" label="标签宽度">
        <el-input v-model.number="activeData.__config__.labelWidth" type="number" placeholder="请输入标签宽度" />
      </el-form-item>
      <el-form-item v-if="activeData.style&&activeData.style.width!==undefined" label="组件宽度">
        <el-input v-model="activeData.style.width" placeholder="请输入组件宽度" clearable />
      </el-form-item>
      <el-form-item v-if="activeData.__vModel__!==undefined && activeData.__config__.tag !== 'el-table'" label="默认值">
        <el-input
      ...
      <el-form-item v-if="activeData.height!==undefined && activeData.__config__.tag !== 'el-table'" label="组件高度">
      ...
      <el-form-item v-if="activeData.direction!==undefined" label="方向">
        <el-select v-model="activeData.direction" placeholder="方向">
          <el-option
      ...
      <el-form-item v-if="activeData['content-position']!==undefined" label="content-position">
        <el-select v-model="activeData['content-position']" placeholder="content-position">
          <el-option
            v-for="(item, index) in contentPositions"
            :key="index"
            :label="item.label"
            :value="item.value"
          />
      ...
      <template v-if="activeData.__config__.methodsList && activeData.__config__.methodsList.length">
        <el-divider>事件</el-divider>
        <el-form-item label="事件类型">
          <el-select v-model="activeData.__config__.method" :style="{ width: '100%' }">
            <el-option
              v-for="(item, index) in activeData.__config__.methodsList"
              :key="index"
              :label="item.label"
              :value="item.value"
            />
        ...
        <el-form-item label="事件名称">
          <el-input
            v-model="activeData.__config__.methodName"
            placeholder="请输入事件名"
          />
        </el-form-item>
        <el-divider />
      ...
      <template v-if="['el-carousel'].indexOf(activeData.__config__.tag) > -1">
        ...
        <draggable
          ...
          handle=".option-drag"
        >
          <div v-for="(item, index) in activeData.__slot__.carouselItems" :key="index" class="select-item">
            <div class="select-line-icon option-drag">
              <i class="el-icon-s-operation" />
            </div>
            <el-input v-model="item.label" placeholder="选项值" size="small" />
            <div class="close-btn select-line-icon" @click="activeData.__slot__.carouselItems.splice(index, 1)">
              <i class="el-icon-remove-outline" />
            </div>
          </div>
        </draggable>
      ...
      <template v-if="['el-table'].indexOf(activeData.__config__.tag) > -1">
        <el-divider>选项</el-divider>
        <el-form-item label="数据类型">
          <el-radio-group v-model="activeData.__config__.tbDataType" size="small">
            <el-radio-button label="dynamic">
              动态数据
            </el-radio-button>
            <el-radio-button label="static">
        ...
        <template v-if="activeData.__config__.tbDataType === 'dynamic'">
          <draggable
            :list="activeData.__slot__.elTableColumn"
            :animation="340"
            group="tableColumnItem"
            handle=".option-drag"
          >
            <div v-for="(item, index) in activeData.__slot__.elTableColumn" :key="index" class="select-item">
              <div class="select-line-icon option-drag">
                <i class="el-icon-s-operation" />
              </div>
              <el-input v-model="item.prop" placeholder="表格项prop" size="small" clearable />
              <el-input v-model="item.label" placeholder="表格项label" size="small" clearable />
              <div class="close-btn select-line-icon" @click="removeTableColumn(index, item.prop)">
                <i class="el-icon-remove-outline" />
              </div>
            </div>
          </draggable>
        ...
        <!-- 级联选择静态数据 -->
        <template v-if="activeData.__config__.tbDataType !== 'dynamic'">
          <el-collapse v-model="activeData.data.size">
            <el-collapse-item
              v-for="(item, index) in activeData.data"
              :key="index"
              :title="`第${index + 1}列表格数据`"
              :name="index"
            >
              <el-form-item
                v-for="(cell, cellIndex) in activeData.__slot__.elTableColumn"
                :key="cellIndex"
                :label="cell.label"
              >
                <el-input v-model="item[cell.prop]" :placeholder="`表格项${cell.label}`" size="small" clearable />
              </el-form-item>
              <div class="close-btn table-column-icon" @click="activeData.data.splice(index, 1)">
                <i class="el-icon-remove-outline" />
                <span>删除当前列</span>
              </div>
            </el-collapse-item>
          </el-collapse>
          <div v-if="activeData.__config__.tbDataType === 'static'" style="margin-left: 20px">
            <el-button
              style="padding-bottom: 0"
              icon="el-icon-circle-plus-outline"
              type="text"
              @click="addTableColumnItem"
            >
              添加表格数据
            </el-button>
          </div>
      ...
      <!-- 设置独立表格（表格不在表单里面） -->
      <!-- <template>
        <el-form-item v-if="activeData.__config__.ownTable !== undefined" label="独立表格">
          <el-switch v-model="activeData.__config__.ownTable" @change="changeOwnTable" />
          <span class="text-remarks">(独立的元素层级，非嵌入form)</span>
        </el-form-item>
      </template> -->

      <el-form-item v-if="activeData.stripe !== undefined" label="斑马纹">
        <el-switch v-model="activeData.stripe" />
      </el-form-item>

      <el-form-item v-if="activeData.border !== undefined" label="边框表格">
        <el-switch v-model="activeData.border" />
      </el-form-item>

      <el-form-item v-if="activeData.height !== undefined && activeData.__config__.tag === 'el-table'" label="固定表头">
        <el-input v-model="activeData.height" placeholder="设置固定表格高度" clearable />
      </el-form-item>

      <el-form-item v-if="activeData.clickName !== undefined" label="点击事件">
        <el-input v-model="activeData.clickName" placeholder="设置点击事件名字" clearable />
      </el-form-item>

      <el-form-item v-if="activeData.__slot__ && activeData.__slot__.default !== undefined" label="按钮名字">
        <el-input v-model="activeData.__slot__.default" placeholder="按钮名字" clearable />
      </el-form-item>
      ...
      <el-form-item v-if="activeData.__config__.tag === 'el-select'" label="创建条目">
        <el-switch v-model="activeData.allowCreate" />
      </el-form-item>
      <el-form-item v-if="activeData.__config__.tag === 'el-select'" label="回车选择">
        <el-switch v-model="activeData.defaultFirstOption" />
      </el-form-item>
      <el-form-item v-if="activeData.__config__.required !== undefined" label="是否必填">
        <el-switch v-model="activeData.__config__.required" />
      </el-form-item>
      <el-form-item v-if="activeData.__config__.tag === 'el-select'" label="自定义html">
        <el-input v-model="activeData.__slot__.html" type="textarea" :rows="5" placeholder="请输入内容" />
      </el-form-item>
      ...
      <template v-if="activeData.__config__.layout === 'colFormItem' && activeData.__config__.tag !== 'el-table'">
...
<script>
import {
  inputComponents, selectComponents, layoutComponent
...
export default {
  ...
  methods: {
    ...
    addCarouselItem() {
      this.activeData.__slot__.carouselItems.push({
        label: ''
      })
    },
    // 添加表格子选项
    addtableItem() {
      this.activeData.__slot__.elTableColumn.push({
        label: '',
        prop: ''
      })
    },
    removeTableColumn(index, prop) {
      this.activeData.__slot__.elTableColumn.splice(index, 1)
      this.activeData.data && this.activeData.data.length && this.activeData.data.forEach(item => {
        delete item[prop]
      })
    },
    addTreeItem() {
    ...
    addTableColumnItem() {
      const obj = {}
      this.activeData.__slot__ && this.activeData.__slot__.elTableColumn.forEach(item => {
        obj[item.prop] = null
      })
      this.activeData.data.push(obj)
    },
    addTableColumnConfirm(data) {
